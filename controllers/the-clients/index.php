...
if (session_db::is_logged()) {
    ...
    if (!$controller_to_include) {
        if (session_db::check_user_level(['god', 'admin'])) {
            ...
            $client_agency_id = session_db::get_user_data()['client_id'] . '--' . session_db::get_user_data()['agency_id'];
            $get_params = ["auth-code" => md5(\k1lib\K1MAGIC::get_value() . $client_agency_id)];
            $go_url = url::do_url(APP_URL . client_clients_config::ROOT_URL . '/' . client_clients_config::BOARD_READ_URL . '/' . $client_agency_id . "/", $get_params);
        } else {
            trigger_error("No idea how you do it!", E_USER_ERROR);
        }
        \k1lib\html\html_header_go($go_url, $get_params);
    } else {
