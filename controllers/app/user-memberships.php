...
DOM::menu_left()->set_active('nav-user-memberships');

$db_table_to_use = "user_memberships";
$controller_name = "User Memberships";
...
$controller_object->set_config_from_class('\k1app\user_memberships_config');
//
///**
...
if ($controller_object->on_object_read()) {
    ...
    // Membership LINK
    $membership_url = url::do_url(APP_BASE_URL . memberships_config::ROOT_URL . '/' . memberships_config::BOARD_READ_URL . '/--customfieldvalue--/', $get_params);
    $controller_object->object_read()->apply_link_on_field_filter($membership_url, ['membership_id'], ['membership_id']);
}
...
if ($controller_object->on_board_read()) {
     ...
     * Related list
     */
    $related_db_table = new \k1lib\crudlexs\class_db_table($db, "membership");
    $controller_object->board_read_object->set_related_show_all_data(FALSE);
    $related_list = $controller_object->board_read_object->create_related_list($related_db_table, NULL, "Memberships", memberships_config::ROOT_URL, memberships_config::BOARD_CREATE_URL, memberships_config::BOARD_READ_URL, memberships_config::BOARD_LIST_URL, TRUE);
