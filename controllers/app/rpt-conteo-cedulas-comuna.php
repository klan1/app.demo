...
DOM::menu_left()->set_active('nav-rpt-conteo-comuna');
...
$content->append_h1("Conteo de cedulas por Comuna en Cali");
$content->set_class("tablero");
$content->append_a('./?download-rpt', 'Descargar reporte', '_self', 'button');

$sql_query = "SELECT * FROM view_conteo_cedulas_comuna";

$reporte_data = \k1lib\sql\sql_query($db, $sql_query, TRUE, TRUE);

if ($reporte_data) {
    if (isset($_GET['download-rpt'])) {
        \k1lib\xml\do_xml($reporte_data, TRUE, 'conteo-de-cedulas-por-zona-' . time() . '.xml');
    } else {
