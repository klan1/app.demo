...
$content->append_h1("Conteo de cedulas por Puesto en Cali"); 
$content->set_class("tablero");

$content->append_a('./?download-rpt', 'Descargar reporte', '_self', 'button');

$sql_query = "SELECT * FROM view_conteo_cedulas_puesto";
...
if ($reporte_data) {
    ...
    if (isset($_GET['download-rpt'])) {
        \k1lib\xml\do_xml($reporte_data, TRUE, 'conteo-de-cedulas-por-puesto-' . time() . '.xml');
    } else {
